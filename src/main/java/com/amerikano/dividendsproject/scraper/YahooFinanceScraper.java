...
import com.amerikano.dividendsproject.model.constants.Month;
    ...
    public ScrapedResult scrap(Company company) {
        ...
        try {
            ...
            for (Element e : tbody.children()) {
                ...
                int month = Month.strToNumber(splitText[0]);
                int day = Integer.parseInt(splitText[1].replace(",", ""));
                int year = Integer.parseInt(splitText[2]);
                String dividend = splitText[3];

                if (month < 0) {
                    throw new RuntimeException("Unexpected Month enum value ->" + splitText[0]);
                }
